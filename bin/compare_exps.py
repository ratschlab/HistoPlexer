...
# keywords = {'cyclegan-MP': 'tupro-patches_ours-FM_channels-all_seed'}
keywords = {'FM-MP': 'tupro-patches_ours-FM_channels-all_seed'}
...
for exp_name in keywords.keys(): 
    ...
    for exp in exps:
        ...
        eval_path = glob.glob(exp + '/test_eval/*/' + 'all_metrics_per_sample.csv')[0]
        print(eval_path)
        # if not os.path.exists(eval_path):
        #     print('File does not exist')
        #     continue
        df_exp = pd.read_csv(glob.glob(exp + '/test_eval/*/' + 'all_metrics_per_sample.csv')[0])
