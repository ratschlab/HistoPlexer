...
def create_tissue_mask(wsi, seg_level, tile_level):
    ...
    scale_factor = round(wsi.level_downsamples[seg_level] / wsi.level_downsamples[tile_level])
...
def create_tissue_tiles(
    wsi, tissue_mask_scaled, tile_size_microns, tile_level=1, offsets_micron=None
):
    ...
    # Compute the tile size in pixels from the desired tile size in microns and the image resolution
    mpp_x = round(float(wsi.properties[openslide.PROPERTY_NAME_MPP_X]), 2) * round(wsi.level_downsamples[tile_level])
    mpp_y = round(float(wsi.properties[openslide.PROPERTY_NAME_MPP_Y]), 2) * round(wsi.level_downsamples[tile_level])
    mpp_scale_factor = min(mpp_x, mpp_y)
    tile_size_pix = round(tile_size_microns / mpp_scale_factor)
    print(f"tile size is {tile_size_pix} pixel at desired level {tile_level}")
    ...
    # Scale the tiles to match the coordinate space of the slide's highest level (level 0)
    scale_factor = wsi.level_downsamples[tile_level]
    filtered_tiles = [scale(rect, xfact=scale_factor, yfact=scale_factor, zfact=1.0, origin=(0, 0)) for rect in filtered_tiles]

    return filtered_tiles, tile_size_pix
...
def crop_rect_from_slide(slide, rect, tile_level, tile_size_pix):
    ...
    return slide.read_region(top_left_coords, tile_level, (tile_size_pix, tile_size_pix))

class BagOfTiles(Dataset):
    def __init__(self, wsi, tiles, tile_level, tile_size_pix, resize_to=224):
        self.wsi = wsi
        self.tiles = tiles
        self.tile_level = tile_level
        self.tile_size_pix = tile_size_pix
    ...
    def __getitem__(self, idx):
        tile = self.tiles[idx]
        img = crop_rect_from_slide(self.wsi, tile, self.tile_level, self.tile_size_pix)
...
def load_encoder(checkpoint_file, use_imagenet_weights=False, device='cuda:0'):
    import torch
    import torchvision

    # model and model code from: https://github.com/ozanciga/self-supervised-histopathology
    ...
    if use_imagenet_weights==True: 
        # expected that checkpoint_file contains resnet18 weights
        model.load_state_dict(state)
        model = torch.nn.Sequential(*(list(model.children())[:-1]))
    else: 
        # used when loading ssl histology resnet model
...
def extract_features(model, device, wsi, filtered_tiles, tile_level, tile_size_pix, workers, out_size, batch_size):
    # Use multiple workers if running on the GPU, otherwise we'll need all workers for
    # evaluating the model.
    kwargs = (
        {"num_workers": workers, "pin_memory": True} if "cuda" in device else {}
    )
    loader = DataLoader(
        dataset=BagOfTiles(wsi, filtered_tiles, tile_level, tile_size_pix, resize_to=out_size),
    ...
    with torch.no_grad():
        for batch, coords in loader:
            ...
            features = model(batch).squeeze().cpu().numpy()
